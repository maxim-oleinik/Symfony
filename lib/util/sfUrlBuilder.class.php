...
/**
 * sfUrlBuilder - URL generation from everything
 *
 * @package    symfony
 * @subpackage util
 */
class sfUrlBuilder
{
   ...
   */
  public static function generateUrl(sfRouting $routing, $parameters = array(), $absolute = false)
  {
    $route = '';
    $fragment = '';
    ...
    if (is_string($parameters))
    {
      ...
      list($route, $parameters) = self::convertUrlStringToParameters($parameters);
    }
    ...
    $url = $routing->generate($route, $parameters, $absolute);
   ...
   */
  public static function convertUrlStringToParameters($url)
  {
    $givenUrl = $url;
